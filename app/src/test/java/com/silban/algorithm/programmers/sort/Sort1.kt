...
class Sort1 {
    private fun solution(array: IntArray, commands: Array<IntArray>): IntArray {
        ...
        commands.forEach {
            val startIndex = it[0] - 1
            val endIndex = it[1]
            val searchCount = it[2] - 1
            val answer = array.toList().subList(startIndex, endIndex).sorted()[searchCount]
            answers.add(answer)
        }
    ...
    private fun bestSolution(array: IntArray, commands: Array<IntArray>): IntArray {
        return commands.map { command ->
            ...
            val searchCount = command[2] - 1
            array.slice(IntRange(startIndex, endIndex)).sorted()[searchCount]
        }.toIntArray()
    }
